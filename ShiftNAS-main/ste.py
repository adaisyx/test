...
import shiftutils as shiftutils

class RoundPowerOf2(Function):
    @staticmethod 
    def forward(ctx, input, stochastic=False):
        return shiftutils.round_power_of_2(input, stochastic)
...
class RoundFixedPoint(Function):
    ...
    def forward(ctx, input, act_integer_bits=16, act_fraction_bits=16):
        return shiftutils.round_to_fixed(input, act_integer_bits, act_fraction_bits)
...
class RoundFunction(Function):
    ...
    def forward(ctx, input, rounding='deterministic'):
        # print(rounding)
        return shiftutils.round(input, rounding)
